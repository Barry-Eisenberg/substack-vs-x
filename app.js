...
/* ============================================================
   QUOTE LINKS — Exodus cards → Quotes carousel
   ============================================================ */
document.querySelectorAll('[data-quote-index]').forEach(el => {
  el.addEventListener('click', () => {
    const idx = parseInt(el.dataset.quoteIndex);
    const voicesSection = document.getElementById('voices');
    if (voicesSection) {
      voicesSection.scrollIntoView({ behavior: 'smooth' });
      setTimeout(() => goToQuote(idx), 600);
    }
  });
});

/* ============================================================
   SOURCE DATA POPOVER SYSTEM
   ============================================================ */
let activePopover = null;

function createSourcePopover(el) {
  removeActivePopover();

  const name = el.dataset.sourceName;
  ...
  pop.innerHTML = `
    <div class="source-popover-label">Data Source</div>
    <div class="source-popover-name">${name}</div>
    <a class="source-popover-link" href="${url}" target="_blank" rel="noopener noreferrer">
      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M6 3H3v10h10v-3"/><path d="M9 2h5v5"/><path d="M14 2 7 9"/>
      </svg>
      View source
    </a>
  `;
  ...
  // Adjust after render if overflows
  requestAnimationFrame(() => {
    ...
    if (popRect.right > window.innerWidth - 10) {
      pop.style.left = (window.innerWidth - popRect.width - 16 + scrollX) + 'px';
      pop.style.transform = 'none';
    }
    // If overflows left
    if (popRect.left < 10) {
      pop.style.left = (16 + scrollX) + 'px';
      pop.style.transform = 'none';
    }
  });

  // Prevent the link click from closing the popover
  pop.addEventListener('click', e => e.stopPropagation());

  activePopover = { el, pop };
}
...
// Delegate click to all elements with data-source-url
document.addEventListener('click', (e) => {
  const sourceEl = e.target.closest('[data-source-url]');

  if (sourceEl) {
    // If this element also has a data-quote-index, let the quote handler fire instead
    if (sourceEl.dataset.quoteIndex !== undefined) return;

    e.stopPropagation();

    // Toggle if clicking same element
    if (activePopover && activePopover.el === sourceEl) {
      removeActivePopover();
      return;
    }
    createSourcePopover(sourceEl);
    return;
  }
...
// Dismiss on scroll for cleanliness
let popScrollTimer;
window.addEventListener('scroll', () => {
  if (activePopover) {
    clearTimeout(popScrollTimer);
    popScrollTimer = setTimeout(() => removeActivePopover(), 150);
  }
}, { passive: true });
